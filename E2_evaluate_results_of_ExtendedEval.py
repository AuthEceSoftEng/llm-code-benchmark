...
def has_executable_tests(raw_tests: Any) -> bool:
    """Check if tests are in executable format (string with check function)"""
    return isinstance(raw_tests, str) and "def check(" in raw_tests


def build_executable_test_harness(llm_code: str, test_code: str, entry_point: str) -> str:
    """Build test harness for executable test format (check function)"""
...
def __run_tests__():
    ...
    # Try to get the function by entry_point name first
    if '{entry_point}' in current_globals:
        user_function = current_globals['{entry_point}']
    else:
        # Find any user-defined function
        for name, obj in current_globals.items():
            if (isinstance(obj, types.FunctionType) and 
                not name.startswith('__') and 
                name not in ['check', '__run_tests__']):
                user_function = obj
                break
    
    if user_function is None:
        raise AssertionError("No user-defined function found")
    
    # Run the check function
    try:
        check(user_function)
        print("__RESULT__: ALL TESTS PASSED")
...
def run_file_and_capture(py_code: str, timeout: float) -> Tuple[bool, str]:
    ...
    try:
        ...
        if proc.returncode == 0 and ("__RESULT__" in out or "ALL TESTS PASSED" in out):
            return True, out
        else:
            return False, out
...
def main():
    ...
    with open(results_file, "r", encoding="utf-8") as f_in, open(report_file, "w", encoding="utf-8") as f_out:
        for line in f_in:
            ...
            # 3) Handle different test formats
            raw_tests = raw.get("test")
            if not raw_tests:
                if args.verbose:
                    print(f"⚠️  Skipping index {dataset_index}: no tests")
                continue
            ...
            # 4) Choose test harness based on test format
            if has_executable_tests(raw_tests):
                # Use executable test format (your current format)
                full_code = build_executable_test_harness(llm_code, raw_tests, entry_point.strip())
            else:
                # Use structured test format (original format)
                cases = normalize_tests(raw_tests)
                if not cases:
                    if args.verbose:
            ...
            f_out.write(json.dumps({
                ...
                "test_format": "executable" if has_executable_tests(raw_tests) else "structured"
            }, ensure_ascii=False) + "\n")

            # short console output
            if args.verbose:
                status = "✅" if success else "❌"
                test_fmt = "exec" if has_executable_tests(raw_tests) else "struct"
                print(f"{status} index {dataset_index} ({entry_point}) [{test_fmt}]")
